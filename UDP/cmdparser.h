...
void handleSendData(int currSockFd, char *buff, int len, struct sockaddr* server_addr){
	...
	sendto(currSockFd, sendData, strlen(sendData),0,server_addr,sizeof(struct sockaddr));
	//printf("%s\n",sendData);
	while(sentDataSize<=len){
		recvfrom(currSockFd,recvData,sizeof(recvData),0, server_addr,(socklen_t *)&addr_len);
		if(atoi(strndup(recvData+8,8))-blockNo==0){
			...
			sendto(currSockFd, sendData, min(512, blockSize-sentDataSize)+48 ,0, server_addr,sizeof(struct sockaddr));
//			fflush(stdout);
...
int handleRecvData(int currSockFd, char **retVal, struct sockaddr *client_addr ){
	setBlocking(currSockFd);
	int addr_len=sizeof(struct sockaddr);
	char recvData[1024], sendData[1024];
	recvfrom(currSockFd,recvData,1024,0,client_addr,(socklen_t *)&addr_len);
//	printf("Recv:%s\n",recvData);
...
//	printf("BigBuffSize %d\n", bigBuffSize);
	...
	strncpy(sendData+4,"0000",4);
	sprintf(sendData+8,"%08d",blockNo);
	sendto(currSockFd, sendData, strlen(sendData),0,client_addr,sizeof(struct sockaddr));
//	printf("%s\n",sendData);
	while(recvDataSize<bigBuffSize){
		int tempSize=recvfrom(currSockFd,recvData,sizeof(recvData),0,client_addr,(socklen_t *)&addr_len);
//		printf("RecvSize:%d\n",tempSize);
//		printf("RecvData:%s\n",recvData);
...
void handleRequest(int currSock, char *inputStr, struct sockaddr *server_addr){
	int addr_len=sizeof(struct sockaddr);
	char **outputStr,*temp;
	int i, result=0;
	//int cmdLength;
	...
	if(!strcmp(outputStr[0],"Exit")){
		close(sock);
		if(currentSock){
			sendto(*currentSock,"q",1, 0,server_addr, sizeof(struct sockaddr)); 
			close(*currentSock);
		}
		exit(0);
	}
	if(!strcmp(outputStr[0],"IndexGet")){
		...
		if(result){
			...
			sendto(currSock, sendData, strlen(sendData), 0,server_addr, sizeof(struct sockaddr));
			char *retVal;
			handleRecvData(currSock, &retVal, server_addr);
			printf("%s",retVal);
			fflush(stdout);
			free(retVal);
		}
		...
		sendto(currSock, sendData, strlen(sendData)+1, 0,server_addr,sizeof(struct sockaddr));
		setBlocking(currSock);
		char recvData[1024];
		recvfrom(currSock,recvData,1024,0,server_addr,(socklen_t *)&addr_len);
		...
		while(recievedSize<fileSize){
			char *retVal;
			tempSize=handleRecvData(currSock, &retVal, server_addr);
		...
		if(!strncmp(recvData+8,"Yes",3)){
			...
			while(1){
				...
				if(bytesRead<=0) break;
					...
					handleSendData(currSock,fileBuff,bytesRead, server_addr);
				}
				free(fileBuff);
			}
		}
		...
		printf("\rInvalid Command. Type help to get the list of commands.\n");
		fflush(stdout);
	}
}
...
void cmdParser(int currSock, char *inputStr, struct sockaddr *client_addr){
	printf("%s\n",inputStr);
	int addr_len=sizeof(struct sockaddr);
	char **outputStr,*temp;
	int i;
	...
	if(!strncmp(inputStr,"CMD_",4)){
		...
		if(!strcmp(outputStr[0],"Exit")){
			close(sock);
			if(currentSock){
				sendto(*currentSock,"q",1, 0,client_addr,sizeof(struct sockaddr)); 
				close(*currentSock);
			}
			exit(0);
		}
		if(!strcmp(outputStr[0],"IndexGet")){
			if(!strcmp(outputStr[1],"ShortList")){
				...
				handleSendData(currSock,result, strlen(result),client_addr);
			}
			if(!strcmp(outputStr[1],"LongList")){
				char *result=listing(NULL,NULL,".*");
				handleSendData(currSock,result, strlen(result),client_addr);
			}
			if(!strcmp(outputStr[1],"Regex")){
		...
		setBlocking(currSock);
		sendto(currSock, sendData, strlen(sendData)+1, 0,client_addr,sizeof(struct sockaddr));
		setNonblocking(currSock);
		...
		while(1){
			...
			if(bytesRead<=0) break;
				...
				handleSendData(currSock,fileBuff,bytesRead, client_addr);
			}
			free(fileBuff);
		}
	}
		...
		if(reply[0]=='Y'){
			...
			sendto(currSock, sendData, strlen(sendData)+1, 0,client_addr,sizeof(struct sockaddr));
			setBlocking(currSock);
			recvfrom(currSock,recvData,1024,0,client_addr,(socklen_t *)&addr_len);
			long long fileSize=atoi(recvData+8),recievedSize=0, tempSize;
			setNonblocking(currSock);
			FILE *fp=fopen(inputStr+8,"wb");
			while(recievedSize<fileSize){
				char *retVal;
				tempSize=handleRecvData(currSock, &retVal, client_addr);
				fwrite(retVal, tempSize, 1, fp);
				recievedSize+=tempSize;
				free(retVal);
			}
